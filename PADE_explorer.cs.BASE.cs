...
namespace PADE
{
    ...
    public partial class PADE_explorer : Form
    {
        ...
        private bool initFileChosen = false;
        ...
        private void treeView1_NodeClicked(object sender, System.Windows.Forms.TreeNodeMouseClickEventArgs args)
        {
            if (args.Button == MouseButtons.Left)
            {
                ...
                {
                    activatePADE(tempNode.Nodes["PADE_sn"].Nodes[0].Text);
        ...
        private void loadInitFileButton_Click(object sender, EventArgs e)
        {
            //the user wants to upload an init file.  Create a popup box to ask for a pathname.
        ...
        /// <summary>
        /// This subroutine adds a PADE to the PADE_List, among other things.
        /// </summary>
        /// <param name="PADEList">The pade to add.</param>
        /// 
        public static void initPADES(PADE newPADE)
        {
            PADE thisPADE = new PADE();
            thisPADE = newPADE;
            int index = TB4.PADE_List.Count + 1;

            if (index == 1) //this is the first PADE
            { thisPADE.PADE_is_MASTER = true; thisPADE.PADE_is_SLAVE = false; }
            else
            {
                for (int i = 1; i <= TB4.PADE_List.Count; i++)
                {
                    if (TB4.PADE_List[i].flgEther_comms && TB4.PADE_List[i].PADE_is_MASTER)
                    { thisPADE.PADE_is_MASTER = false; thisPADE.PADE_is_SLAVE = true; }
                    else
                    { thisPADE.PADE_is_MASTER = true; thisPADE.PADE_is_SLAVE = false; }
                }
            }
            ...
            //TB4.thePAD_selector.AddPAD(index, false); this creates some errors...I'll need to work on it

            thisPADE.flgEther_comms = true;
            thisPADE.flgUSB_comms = false;
        }

        /// <summary>
        /// This subroutine is used for debugging.  It just creates a bunch of PADES.
        /// </summary>
        public void createPADES()
        {
            for (int i = 0; i < 20; i++)
            {
                PADE newPADE = new PADE(i);
                newPADE.PADE_sn = (i + 1).ToString();
                newPADE.PADE_descr = "PADE #" + (i + 1).ToString();
                initPADES(newPADE);
            }
        }
        ...
        {
            //If no initialization file has been chosen, kindly reject the caller.
            if(!initFileChosen){
                this.toolStripStatusLabel1.Text = "Initialization Failed: No Init File Uploaded.";
                MessageBox.Show("Please upload an initialization file first."); 
                return;
            }

            updateStatusBar(true, 0);

            for(int i=1; i <= PADEList.Length; i++){ //iterate through all PADE's
        ...
        public static void activatePADE(string SN)
        {
            ...
            if (activePADE != null)
            {
                try
                {
                    TB4.activatePADE(activePADE, false);
                }
                catch (Exception ex)
                {
        ...
        public tempStamp getTemperature(PADE targetPADE)
        {
            ...
            TB4.activatePADE(targetPADE, false);
        ...
        public bool searchForPADE(string SN)
        {
            ...
            if (verReg.RegRead() > 0)
            {
                //successful read, add to PADE_List!
        ...
        public void findAllPADES()
        {
            for (int i = 1; i <= 100; i++)
            {
                if (searchForPADE(i.ToString()))
                {
                    ...
                    newPADE.IP4_add[0] = newPADE.MAC_add[0] =  (byte)i;
        ...
        private void registerBaseUnitToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            newBox.Location = new Point(36, 12);

            Button newButton = new Button();
            newButton.Width = 50;
            newButton.Height = 25;
            newButton.Text = "Accept";
            newButton.Location = new Point(36, 40);

            Form answerForm = new Form();
            answerForm.Width = 150;
